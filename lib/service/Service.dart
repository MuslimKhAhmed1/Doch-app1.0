...
class ApiService {
  ...
  Future<List<Site>> fetchSites() async {
    try {
      print('Fetching from: $baseUrl/sites');
      ...
      if (response.statusCode == 200) {
        ...
        print('Number of sites found: ${sites.length}');
        return sites.map((json) => Site.fromJson(json)).toList();
      } else {
        print('Error response: ${response.body}');
        throw Exception('Failed to load sites: Status ${response.statusCode}');
      }
    } catch (e) {
      print('Exception in fetchSites: $e');
...
class Site {
  ...
  // Updated Site.fromJson method to handle null values properly
    ...
    return Site(
      ...
      openHours:
          (json['open_hours'] as List? ?? [])
              ...
              .toList(),
      chModels:
          (json['ch_models'] as List? ?? [])
              .map((e) => ChModel.fromJson(e))
              .toList(),
      siteType:
          json['site_type'] != null
              ? SiteType.fromJson(json['site_type'])
              : SiteType(id: 0, name: 'Unknown'),
    );
  }
}
